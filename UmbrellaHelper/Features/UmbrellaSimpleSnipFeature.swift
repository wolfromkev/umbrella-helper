...
final class UmbrellaSimpleSnipFeature: ObservableObject {
    ...
    func takeFullScreenSnip() {
        ...
        runScreencapture(arguments: ["-x", "-t", "jpg", output], outputPath: output, reveal: revealScreenshotInFinder)
    }
    ...
    private func finishImageSnip(selection: CGRect) {
        let output = screenshotOutputPath()
        captureSelectionRegion(selection, outputPath: output, compressed: true) { [weak self] success in
            guard let self else { return }
            if success {
                self.screenCaptureState = .allowed
                self.lastSavedPath = output
                self.defaults.set(output, forKey: "umbrella.snip.lastSavedPath")
                self.copyImageToClipboardAsync(from: output) { [weak self] copied in
                    guard let self else { return }
    ...
    private func finishTextSnip(selection: CGRect) {
        ...
        captureSelectionRegion(selection, outputPath: output, compressed: false) { [weak self] success in
            guard let self else { return }
            if success {
                self.screenCaptureState = .allowed
    ...
    private func captureSelectionRegion(_ selection: CGRect, outputPath: String, compressed: Bool, completion: @escaping (Bool) -> Void) {
        ...
        Task.detached(priority: .userInitiated) { [weak self] in
            if let image = UmbrellaScreenCoordinateSpace.captureImage(
                fromAppKitSelection: appKitSelection,
                nominalResolution: compressed
            ) {
                let wrote = compressed
                    ? UmbrellaScreenCoordinateSpace.writeJPEG(image, to: outputPath)
                    : UmbrellaScreenCoordinateSpace.writePNG(image, to: outputPath)
                if wrote {
                    await MainActor.run { [weak self] in
                        self?.logDebug("snip capture: CGWindowListCreateImage ok")
                        completion(true)
                    }
                    return
                }
            ...
            let args = UmbrellaScreenCoordinateSpace.screencaptureRegionArguments(for: selection, outputPath: outputPath)
            await MainActor.run { [weak self] in
                self?.logDebug("snip capture fallback: screencapture \(args.joined(separator: " "))")
            }
            ...
            do {
                ...
                var ok = status == 0 && fileExists
                if ok, compressed {
                    ok = UmbrellaScreenCoordinateSpace.recompressScreenshot(at: outputPath)
                }
                await MainActor.run { [weak self] in
                    self?.logDebug("snip exit=\(status) file=\(fileExists ? "yes" : "no")\(stderr.isEmpty ? "" : " stderr=\(stderr)")")
                    completion(ok)
                }
            } catch {
                await MainActor.run { [weak self] in
                    self?.logDebug("snip launch error: \(error.localizedDescription)")
                    completion(false)
                }
            }
        }
        ...
        process.terminationHandler = { [weak self] task in
            ...
            Task { @MainActor in
                ...
                if task.terminationStatus == 0, fileExists {
                    _ = UmbrellaScreenCoordinateSpace.recompressScreenshot(at: outputPath)
                    self.screenCaptureState = .allowed
    ...
    private func screenshotOutputPath() -> String {
        ensureDirectory(screenshotFolderPath)
        let name = "snip-\(Self.timestamp()).jpg"
    ...
    /// Load image off the main thread, then write the pasteboard on the main actor.
    private func copyImageToClipboardAsync(from path: String, completion: @escaping (Bool) -> Void) {
        Task.detached(priority: .userInitiated) {
            ...
            await MainActor.run {
                ...
                let pasteboard = NSPasteboard.general
                pasteboard.clearContents()
                completion(pasteboard.writeObjects([image]))
            }
        }
    }
...
enum UmbrellaScreenCoordinateSpace {
    /// Soften overlay stacking vs `.screenSaver` while staying above normal app chrome.
    static let overlayWindowLevel = NSWindow.Level.popUpMenu
    static let overlayCollectionBehavior: NSWindow.CollectionBehavior = [
        .canJoinAllSpaces,
        .fullScreenAuxiliary,
    ]

    private static var cachedWindowFrames: [NSRect] = []
    private static var windowListCacheTime: CFAbsoluteTime = 0
    private static let windowListCacheInterval: CFAbsoluteTime = 1.0 / 45.0
    ...
    static func captureImage(fromAppKitSelection selection: NSRect, nominalResolution: Bool) -> CGImage? {
        let captureRect = cgWindowBounds(fromAppKit: selection)
        guard captureRect.width > 0, captureRect.height > 0 else { return nil }
        var options: CGWindowImageOption = [.boundsIgnoreFraming]
        options.insert(nominalResolution ? .nominalResolution : .bestResolution)
    ...
    static func writePNG(_ image: CGImage, to outputPath: String) -> Bool {
        ...
        return writeImageData(data, to: outputPath)
    }

    /// JPEG at ~0.75 quality, capped at 2560px on the long edge — readable UI, much smaller than PNG.
    @discardableResult
    static func writeJPEG(_ image: CGImage, to outputPath: String) -> Bool {
        let prepared = downscaled(image, maxDimension: 2560)
        let rep = NSBitmapImageRep(cgImage: prepared)
        guard let data = rep.representation(using: .jpeg, properties: [.compressionFactor: 0.75]) else { return false }
        return writeImageData(data, to: outputPath)
    }

    @discardableResult
    static func recompressScreenshot(at path: String) -> Bool {
        let url = URL(fileURLWithPath: path)
        guard let source = CGImageSourceCreateWithURL(url as CFURL, nil),
              let image = CGImageSourceCreateImageAtIndex(source, 0, nil) else {
            return FileManager.default.fileExists(atPath: path)
        }
        return writeJPEG(image, to: path)
    }

    private static func writeImageData(_ data: Data, to outputPath: String) -> Bool {
        do {
    ...
    private static func downscaled(_ image: CGImage, maxDimension: Int) -> CGImage {
        ...
        let scale = min(CGFloat(maxDimension) / CGFloat(width), CGFloat(maxDimension) / CGFloat(height))
    ...
    static func topmostWindowFrame(at point: NSPoint) -> NSRect? {
        ...
        if cachedWindowFrames.isEmpty || now - windowListCacheTime >= windowListCacheInterval {
            cachedWindowFrames = loadOnScreenWindowFrames()
            windowListCacheTime = now
        }

        for appKitFrame in cachedWindowFrames {
    ...
    private static func loadOnScreenWindowFrames() -> [NSRect] {
        ...
        for info in windowList {
            ...
            frames.append(appKitRect(fromCGWindowBounds: bounds))
        }
        return frames
    }
}
...
final class UmbrellaAreaSelectionOverlay {
    ...
    fileprivate final class OverlayWindow: NSWindow {
        ...
        init(screen: NSScreen, selector: UmbrellaAreaSelectionOverlay) {
            ...
            level = UmbrellaScreenCoordinateSpace.overlayWindowLevel
            isOpaque = false
            ...
            sharingType = .none
            collectionBehavior = UmbrellaScreenCoordinateSpace.overlayCollectionBehavior
...
final class UmbrellaRecordingOverlay {
    ...
    func present() {
        ...
        pulseTimer = Timer.scheduledTimer(withTimeInterval: 0.75, repeats: true) { [weak self] _ in
            guard let self else { return }
            self.pulseOn.toggle()
            // Pulse only the HUD dot — avoid redrawing fullscreen border windows every tick.
    ...
    private final class ControlPanel: NSPanel {
        ...
        init(showsAudioVisualizer: Bool, audioLevelMeter: UmbrellaAudioLevelMeter?, onStop: @escaping () -> Void) {
            ...
            if audioVisualizer != nil {
                visualizerTimer = Timer.scheduledTimer(withTimeInterval: 1.0 / 12.0, repeats: true) { [weak self] _ in
...
final class UmbrellaScreenRecorder: NSObject, SCStreamDelegate {
    ...
    func startRecording(selection: CGRect, config: UmbrellaRecordingConfig) async throws -> URL {
        ...
        let (width, height) = Self.recordingPixelSize(pointWidth: cropRect.width, pointHeight: cropRect.height)
    ...
    /// 1x capture, longest side capped at 1920 — plenty for UI, far smaller than Retina H.264.
    private static func recordingPixelSize(pointWidth: CGFloat, pointHeight: CGFloat) -> (Int, Int) {
        let maxDimension = 1920
        var width = max(2, Int(pointWidth.rounded()))
        var height = max(2, Int(pointHeight.rounded()))
    ...
    private static func videoBitRate(width: Int, height: Int) -> Int {
        ...
        let rate = Int(Double(pixels) * 1.8)
        return min(max(rate, 400_000), 2_500_000)
    }
    ...
    private final class StreamOutput: NSObject, SCStreamOutput {
        ...
        private let audioLevelMeter: UmbrellaAudioLevelMeter
        private weak var parent: UmbrellaScreenRecorder?
