...
struct NotionTaskContentView: View {
    ...
    var body: some View {
        ...
        .onChange(of: model.isLoadingNotionSchema) { _, _ in
            model.refreshNotionTaskLayout()
        }
        .onChange(of: model.notionSchema) { _, _ in
            model.refreshNotionTaskLayout()
            focusedMetadataField = nil
            dueDateShowsActualDate = false
        }
        .onChange(of: focusedMetadataField) { _, field in
            if field != nil {
            ...
            VStack(alignment: .leading, spacing: 6) {
                HStack(spacing: 8) {
                    ...
                    TextField("Add a task to notion", text: $model.notionTaskTitle, axis: .vertical)
                        ...
                        .onChange(of: isTaskFocused) { _, isFocused in
                            if isFocused {
                                focusedMetadataField = nil
                                isSettingsFocused = false
                            }
                    ...
                    HStack(spacing: 6) {
                        ...
                        .onChange(of: isSettingsFocused) { _, focused in
                            if focused {
                                focusedMetadataField = nil
                                isTaskFocused = false
                            }
